...
function Accessories(props) {
  ...
  useEffect(() => {
    ...
    axios.post(`/vendorAccessories`, { data }, {
      headers: {
  ...
  const handleChange = (data) => {
    let flag = 0
    selectedItems.forEach(item => { if (item === data) flag++ })
    if (flag === 0) {
      toast('Item Added');
          ...
          return (
            <>
              ...
              <div className='text-left flex-row'>
                ...
                <button onClick={(e) => {  handleChange(data) }} className='w-1/2 mt-2 bg-bgColor p-2 rounded-full  '>SELECT</button>
              </div>
            </>
          )
        })}
