...
function OtpVerify() {
  ...
  useEffect(() => {
    if (localStorage.getItem("user")) {
      if (
        location.pathname === "/User/Login/Otp/Verify" ||
        location.pathname === "/User/Signup/Otp/Verify"
      ) {
        navigate("/");
      }
    }
    if (localStorage.getItem("vendor")) {
      if (
        location.pathname === "/Vendor/Login/Otp/Verify" ||
        location.pathname === "Vendor/Signup/Otp/Verify"
      ) {
    ...
    <div className="container-fluid">
      <div className="flex justify-center items-center h-screen">
        <div className="bg-boxColor w-10/12 md:w-3/5 h-[600px] rounded-3xl shadow-lg grid md:grid-cols-2">
          <div
            className="bg-green-200 hidden md:block rounded-l-3xl"
            style={{
              backgroundImage: `url("https://res.cloudinary.com/dczou8g32/image/upload/v1678510874/project2/josh-nuttall-pIwu5XNvXpk-unsplash_copy_ia5swd.jpg")`,
          ...
          <form method="post" onSubmit={handleOtp}>
            <div className=" flex flex-col  items-center">
              <h1 className="mt-20 text-3xl sm:text-4xl text-center">
                VERIFY ACCOUNT
              </h1>
              <h1 className="mt-7 sm:text-xl text-center">
                An OTP has been sent to your entered phone number
              </h1>
              <h1 className="mt-9  sm:text-2xl text-center">
              ...
              {state ? (
                <button className="text-white      rounded-full  shadow-lg   mt-12      bg-bgColor hover:bg-bgColor focus:ring-4 focus:ring-blue-300 font-medium  text-sm px-5 py-2.5 text-center mr-2 dark:bg-bgColor dark:hover:bg-[#30444a] dark:focus:ring-bgColor inline-flex items-center">
                  VERIFY
                </button>
              ) : (
                <button
                  disabled
                  type="button"
                  className="text-white      rounded-full  shadow-lg    mt-12     bg-bgColor hover:bg-bgColor focus:ring-4 focus:ring-blue-300 font-medium  text-sm px-5 py-2.5 text-center mr-2 dark:bg-bgColor  dark:focus:ring-bgColor inline-flex items-center"
                >
                  <svg
                    aria-hidden="true"
                    role="status"
