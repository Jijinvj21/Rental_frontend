...
import { accessoriesValidationSchema ,imageValidationSchema } from '../../validation/validation';
...
function UserTable() {
    ...
    function addCycle() {
        ...
        const handleSubmit = async (e) => {
            ...
            imageValidationSchema.validate({ image: image })
                .then(async () => {
                    accessoriesValidationSchema.validate(inputData)
                        .then(async () => {

                            setUplod(false)
                            await axios.post('/vendor/insert_accessories', datas, {
                                headers: {
            ...
            <div className='w-screen '>
                ...
                <div className='bg-boxColor      my-auto w-full flex flex-row justify-center items-center rounded-3xl  ' >
                    <form className='flex flex-col justify-center items-center  w-full  '
                        ...
                        <label class=" bg-bgColor  shadow-lg p-2 m-2 w-4/5 rounded-full cursor-pointer">  {image ? image?.name : 'CHOOSE FILE'}
                            <input
                                name='image'
                                type="file"
                                class="hidden"
                        ...
                        <input className='bg-bgColor  rounded-full   shadow-lg p-2 m-2 w-4/5  focus:outline-none'
                            ...
                            placeholder='SIZE'
                            name="tyresize"
                            value={inputData.name} onChange={(e) => setInputData({ ...inputData, size: e.target.value })} />
                        <input className='bg-bgColor  rounded-full   shadow-lg p-2 m-2 w-4/5  focus:outline-none'
                            type='text'
                        ...
                        {upload ?
                            <button class="text-white      rounded-full  shadow-lg         bg-bgColor hover:bg-bgColor focus:ring-4 focus:ring-blue-300 font-medium  text-sm px-5 py-2.5 text-center mr-2 dark:bg-bgColor dark:hover:bg-[#30444a] dark:focus:ring-bgColor inline-flex items-center">SUBMIT</button> :
                            <button disabled type="button" class="text-white      rounded-full  shadow-lg         bg-bgColor hover:bg-bgColor focus:ring-4 focus:ring-blue-300 font-medium  text-sm px-5 py-2.5 text-center mr-2 dark:bg-bgColor  dark:focus:ring-bgColor inline-flex items-center">
    ...
    user?.map((data, index) => {
        let { image, quantity, size, price, name } = data
        ...
        let table_head = {
            ...
            Status: <button className=' bg-bgColor hover:bg-[#558081]/90  focus:outline-none focus:ring-[#558081]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 dark:hover:bg-[#558081]/30 mr-2 mb-2 '
                onClick={() => {
                    axios.get(`/vendor/accessoriesStatus_Update/${data._id}`, {
                        headers: {
                          'Authorization': 'Bearer ' + token,
